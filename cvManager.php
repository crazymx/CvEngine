...
class CvManager{
	...
	*/
	...
	public function add(Cv $cv){
		$q = $this->_db->prepare('INSERT INTO cv(name, firstname, dob, address, zipcode, city, country, phone, licence, hobbies) VALUES(:name, :firstname, :dob, :address, :zipcode, :city, :country, :phone, :licence, :hobbies)');
	...
	{
		$this->_db->exec('DELETE FROM cv WHERE cvID = '.$cv->cvid());
	}
	...
	{
	    $q = $this->_db->prepare('UPDATE cv SET name = :name, firstname = :firstname, dob = :dob, address = :address, zipcode = :zipcode, city = :city, country = :country, phone = :phone, licence = :licence, hobbies = :hobbies WHERE cvID = '.$cv->cvid());
		...
		$q->bindValue(':phone', $cv->phone(), PDO::PARAM_INT);
